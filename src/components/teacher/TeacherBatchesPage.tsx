...
function BatchAttendanceControls({
  ...
  const locked = batch.todayAttendance.alreadyMarked;
  const displayStatus = locked ? batch.todayAttendance.status : draft.status;
    ...
    return (
      <div className="space-y-1.5 min-w-[200px]">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <span className="text-xs text-amber-700">Marked for today</span>
        </div>
        {batch.todayAttendance.remarks ? (
          <p className="text-xs text-muted-foreground truncate max-w-[220px]" title={batch.todayAttendance.remarks}>
            {batch.todayAttendance.remarks}
          </p>
        ) : null}
      </div>
...
export function TeacherBatchesPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/teacher/batches?${params}`, { credentials: "include" });
  ...
  }, [page, debounced, filterCourse, filterStatus, router]);
      ...
      <div className="rounded-3xl bg-white border border-slate-200 shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="py-16 text-center px-6">
            ...
            <p className="font-display font-semibold text-lg">No batches assigned yet</p>
            <p className="text-sm text-muted-foreground mt-2 max-w-md mx-auto">
              When a senior teacher assigns you to a batch, it will appear here automatically.
            </p>
          </div>
        ) : (
          <>
            <div className="hidden md:block overflow-x-auto">
              <Table>
                ...
                <TableBody className="divide-y divide-slate-100">
                  {batches.map(b => (
                    <TableRow key={b.id} className="border-0 hover:bg-slate-50 align-top">
                      <TableCell className="px-6 py-4 font-semibold">{b.batchName}</TableCell>
                      <TableCell className="px-6 py-4">{b.courseName}</TableCell>
                      <TableCell className="px-6 py-4">{b.batchTiming || `${b.batchDay} · ${b.batchTime}`}</TableCell>
                      <TableCell className="px-6 py-4">{b.totalStudents}</TableCell>
