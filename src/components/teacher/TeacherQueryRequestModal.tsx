...
const schema = z.object({
  teacherName: z.string().trim().min(2, "Name is required (min 2 characters)"),
  teacherEmail: z.string().trim().email("Enter a valid email address"),
      ...
      <DialogContent className="rounded-3xl max-w-md border-border/80 shadow-xl">
        ...
        >
          <div className="space-y-1.5">
            <Label htmlFor="tq-name">Name</Label>
            <Input id="tq-name" className="rounded-xl" {...register("teacherName")} />
            {errors.teacherName && (
          ...
          <div className="space-y-1.5">
            <Label htmlFor="tq-email">Email</Label>
            <Input id="tq-email" type="email" className="rounded-xl" {...register("teacherEmail")} />
            {errors.teacherEmail && (
              <p className="text-xs text-destructive">{errors.teacherEmail.message}</p>
            )}
          </div>

          <div className="space-y-1.5">
            <Label htmlFor="tq-remarks">Remarks / Query message</Label>
            <Textarea
              id="tq-remarks"
              rows={4}
