...
function queryDetailsHtml(fields: {
  ...
  return `
    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border-radius:14px;background:#f8fafc;border:1px solid #e2e8f0;margin:16px 0;">
      <tr><td style="padding:12px 16px;border-bottom:1px solid #e2e8f0;"><strong>Teacher</strong><br/>${escapeHtml(fields.teacherName)}</td></tr>
      <tr><td style="padding:12px 16px;border-bottom:1px solid #e2e8f0;"><strong>Email</strong><br/>${escapeHtml(fields.teacherEmail)}</td></tr>
...
export async function sendNewTeacherQueryEmails(fields: {
  ...
  const html = wrapEmail(
    "New Teacher Query Request",
    `<p>A teacher has submitted a new profile edit query.</p>${queryDetailsHtml({
      ...fields,
      status: "Pending",
    })}`,
  );
  const text = `New teacher query from ${fields.teacherName} (${fields.teacherEmail}). Remarks: ${fields.remarks}. Status: Pending`;

  for (const to of recipients) {
    try {
      await sendTransactionalEmail({ to, subject: "New Teacher Query Request", html, text });
    } catch (e) {
      console.error("[teacher query email admin]", to, e);
      warnings.push(to);
    }
  }
  return warnings;
